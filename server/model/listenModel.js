...
//获取所有课程
ListenSchema.statics.get_all_file = function (options, page, num) {
...
//获取所有已发布
ListenSchema.statics.get_all_public = function (options, page, num) {
    return this.find(options).sort({updateTime:-1}).exec();
};

//获取所有课程数量
ListenSchema.statics.get_all_num = function (options) {
    return this.find(options).exec();
};

//删除一个课程
ListenSchema.statics.delete = function (id) {
    return this.remove({_id: id}).exec();
};

//更新
ListenSchema.statics.update_lesson = function (options, id) {
    return this.update({_id: id}, options).exec();
};

//根据id获取一个课程
ListenSchema.statics.get_one = function (id) {
...
ListenSchema.statics.get_all_pop = function (options) {
    return this.find(options)
        .populate('materialId')
        .sort({updateTime:-1}).exec();
};

module.exports = db.conn.model('Listen', ListenSchema);
