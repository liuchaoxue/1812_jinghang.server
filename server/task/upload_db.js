...
//         var type = typeMap[material.fileUrl.split('/').pop().split('.').pop()]
//         let updateInfo = {
//             fileUrl: config.host+'/v1/media/'+material._id +'.'+ type,
//             files:[{'r':'default', file_id:file_id, type: type }],
//             cdnUrl: false,
//             mark: (material.source||'upload') +"_"+ material.fileUrl.split('/').pop().split('.')[0]
//         };
...
//                 if(!material) return ;
//                 cms = cms.replace(fileList[index], config.host+'/v1/media/'+material._id+"."+fileList[index].split('.').pop())
//             })
//             let updateInfo = {
//                 cms : cms
//
...
function updateOneMaterialModel(material) {
    ...
    return new Promise(resolve=>{
        ...
        let updateInfo = {
            fileUrl: material.fileUrl.replace('localhost:4000','cms.jinghangapps.com'),
        };
        MaterialModel.update_material(updateInfo, material._id).then(()=>{
            resolve()
        })
